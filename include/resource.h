...
namespace gfx {
    ...
    public:
        ...
        ~Repository()
        {
            // Signal the watcher loop to exit, then wait for the coroutine to actually
            // reach its final suspend before the Task frees the frame. Destroying the
            // handle while a background worker is still executing inside it (e.g. mid
            // sleep_for) frees the coroutine frame out from under that thread and
            // corrupts the heap. take() is non-blocking, so poll done() here — this runs
