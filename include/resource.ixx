...
public:
    ...
    T& get(std::string_view id) {
        auto& s = storage<T>();
        if (!s) {
            logger::error("Resource of type '{}' not found in repository!", typeid(T).name());
        ...
        if (it == s.items.end()) {
            logger::error("Resource with id '{}' not found in repository!", id);
    ...
    ResourceRef<const T> add(std::string_view id, Resource<T> resource) {
        ...
        if (s.items.contains(std::string(id))) {
            logger::error("Resource with id '{}' already exists in repository!", id);
...
void Resource<ResourceType>::AddToRepository() {
    Repository().addRef(ResourceRef<ResourceType>(*this));
}
