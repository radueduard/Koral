...
TEST(ImageFormat, BlockSizesAreEightOrSixteenBytes) {
    ...
    for (const auto format : { Format::eBC1_RGB_UNORM, Format::eBC1_RGBA_UNORM, Format::eBC4_UNORM,
        ...
        EXPECT_EQ(Image::blockSize(format), 8u) << "format " << static_cast<int>(format);
    }
    ...
    for (const auto format : { Format::eBC2_UNORM, Format::eBC3_UNORM, Format::eBC5_UNORM,
        ...
        EXPECT_EQ(Image::blockSize(format), 16u) << "format " << static_cast<int>(format);
    }
}
...
TEST(ImageFormat, ChannelSizeRefusesCompressedFormats) {
    EXPECT_THROW((void)Image::channelSize(Format::eBC7_UNORM), std::runtime_error);
    EXPECT_THROW((void)Image::channelCount(Format::eASTC_4x4_UNORM), std::runtime_error);
}
