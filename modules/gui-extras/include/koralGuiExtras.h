...
namespace kgui
{
    ...
    public:
        ...
        explicit GradientEditor(const std::vector<ImVec4>& colors)
        {
            ...
            if (colors.empty())
            {
                _points.push_back({ 0.f, {0,0,0,1} });
                _points.push_back({ 1.f, {1,1,1,1} });
            }
            else
            {
                ...
                for (std::size_t i = 0; i < colors.size(); ++i)
                {
                    ...
                    _points.push_back({ i * step, colors[i] });
                }
                Sort();
            }
        }
        ...
        // ---- Accessors ----------------------------------------------------------
        const std::vector<GradientPoint>& Points() const { return _points; }
              std::vector<GradientPoint>& Points()        { return _points; }

        // Sample the gradient at t in [0,1]
        ImVec4 Sample(float t) const { return SamplePoints(_points, t); }
        ...
        bool Draw(const char* label, float width = 0.f)
        {
            ...
            auto HitHandle = [&]() -> int          // returns index or -1
            {
                ...
                {
                    float dx = mp.x - PosToX(_points[i].position);
            ...
            // --- Drag continuation -----------------------------------------------
            // _dragIdx is set when a drag starts and cleared on mouse-up.
            ...
            if (_dragIdx >= 0)
            {
                ...
                {
                    _points[_dragIdx].position = XToPos(mp.x);
                    // Re-sort and keep _dragIdx pointing at the same element.
            ...
            if (stripHovered && _dragIdx < 0)
            {
                ...
                if (h >= 0)
                {
                    ...
                    if (ImGui::IsMouseClicked(ImGuiMouseButton_Right)
                        && _points.size() > 1)
                    {
                        _points.erase(_points.begin() + h);
                        _selectedIdx = std::clamp(_selectedIdx,
                                                0, (int)_points.size() - 1);
                        modified = true;
                    }
            ...
            // Gradient quads
            {
                ...
                for (int i = 0; i < SEGS; ++i)
                {
                    ...
                    ImU32 c0 = ToU32(SamplePoints(_points, t0));
                    ImU32 c1 = ToU32(SamplePoints(_points, t1));
            ...
            for (int i = 0; i < (int)_points.size(); ++i)
            {
                const float hx   = PosToX(_points[i].position);
                const bool  isSel = (i == _selectedIdx);
            ...
            {
                if (_selectedIdx >= 0 && _selectedIdx < (int)_points.size())
                {
                    GradientPoint& pt = _points[_selectedIdx];

                    ImGui::TextDisabled("Stop %d", _selectedIdx);
                    ImGui::Separator();
    ...
    private:
        ...
        int _selectedIdx = 0;
        int _dragIdx     = -1;    // index of point currently being dragged; -1 = none
        ...
        void Sort()
        {
            ...
            float selPos = (_selectedIdx >= 0 && _selectedIdx < (int)_points.size())
                           ? _points[_selectedIdx].position : -1.f;
            ...
            {
                for (int i = 0; i < (int)_points.size(); ++i)
                    if (_points[i].position == selPos) { _selectedIdx = i; break; }
            }
        }

        // Bubble the dragged element one step left/right to maintain sort order
        // while keeping _dragIdx pointing at it — avoids a full re-sort that
        // would lose the drag index.
        void ResortDragged()
        {
            if (_dragIdx < 0) return;
            float pos = _points[_dragIdx].position;

            // Bubble right
            while (_dragIdx + 1 < (int)_points.size() &&
                   _points[_dragIdx + 1].position < pos)
            {
                std::swap(_points[_dragIdx], _points[_dragIdx + 1]);
                ++_dragIdx;
            }
            // Bubble left
            while (_dragIdx - 1 >= 0 &&
